...
**지표 정의·세션 기준·품질 검증을 고정하고, Gate를 통과한 결과만 운영 산출물로 전달하는 파이프라인을 구축하는 것**입니다.
...
| PRE-RAW Gate | STAGING 전 원천 정합성 선검증 | 원천 도메인/트랜잭션 무결성/핵심 null 체크를 통과한 실행만 STAGING 진행 |
...
| PRE-KPI Gate | KPI 산출 직전 마트 정합성 검증 | event/session 일치 + stg/fact null 체크를 통과한 실행만 KPI 계산 진행 |
| KPI | 지표 계산 일관성 확보 | Funnel/Cohort/CRM 계산을 별도 테이블로 분리해 분자/분모 정의를 단일화 |
| POST-KPI Gate | KPI 산출 후 결과 검증 | KPI sanity + source 일치 + monotonic 검증을 통과한 실행만 export 허용 |
| EXPORT | 실행 가능한 전달물 생성 | PRE-RAW/PRE-KPI/POST-KPI Gate 통과 시에만 CSV 3종 + summary TXT 1종을 고정 파일명 패턴으로 생성 |

<details>
<summary><strong>워크플로우 전체 상세 보기 (RAW → PRE-RAW Gate → STAGING → DATA MART → PRE-KPI Gate → KPI → POST-KPI Gate → EXPORT)</strong></summary>
...
### 2) PRE-RAW Gate — STAGING 전 원천 정합성 게이트
...
### 5) PRE-KPI Gate — KPI 직전 마트 정합성 게이트
...
### 7) POST-KPI Gate — KPI 이후 결과 검증 게이트
...
실제로 `scripts/run_quality_checks.py`는 각 게이트에서 하나라도 FAIL이면 비정상 종료(`exit 1`)되며, 이 때문에 PRE-RAW/PRE-KPI/POST-KPI Gate 통과 전에는 CSV/TXT export가 실행되지 않습니다.

### 8) EXPORT — 운영팀이 바로 쓸 수 있는 형태로 마감
- **왜 필요한가**: 분석 결과를 실행 포맷으로 전달하지 않으면 후속 액션이 지연됩니다.
- **어떻게 했나**
  - PRE-RAW/PRE-KPI/POST-KPI Gate 통과 실행에서만 CSV/TXT를 생성
...
- Gate: PRE-RAW / PRE-KPI / POST-KPI 체크 모두 `PASS`
