...
로그 기반 KPI가 흔들리는 현업에서 **세션·지표 정의를 고정하고 QA로 왜곡을 차단해 퍼널·코호트·CRM 산출물을 자동 생성**하도록 설계한 분석가 중심 프로젝트입니다.

## 현업 시나리오
월요일 아침 CVR이 예를 들어 2.4%→1.7%로 급락했다는 알림이 오면, 마케팅/상품/운영팀은 예산 조정과 캠페인 중단 여부를 바로 논의합니다.  
