...
설계 선택:
- 세션 기준 통일: 30분 inactivity + 날짜 변경
- 책임 분리: 지표 정의(분자/분모) vs 전처리/세션화
- 품질 선행: QA 5종 통과 후에만 export
- 전달 포맷 고정: CSV 3종 + summary TXT 1종
- 재현 지원: `target_date`로 과거 날짜 backfill

파이프라인 요약:

| 단계 | 목적 | 주요 테이블/산출물 |
|---|---|---|
| RAW | 원본 로그 보존 | `raw_rr_events`, `raw_rr_item_properties`, `raw_rr_category_tree` |
| STAGING | 타입/포맷 표준화 | `stg_rr_events`, `stg_rr_item_snapshot`, `stg_rr_category_dim` |
| DATA MART | 분석 단위(fact/dim + 세션) 통일 | `dim_rr_*`, `fact_rr_*` |
| KPI | 퍼널·코호트·CRM 계산 분리 | `mart_rr_funnel_daily`, `mart_rr_funnel_category_daily`, `mart_rr_cohort_weekly`, `mart_rr_crm_targets_daily` |
| QA | 품질 기준 검증 | `quality_check_runs` |
| EXPORT | 운영 전달물 생성 | `logs/reports/rr_*.{csv,txt}` |

모델링/검증 요약:
- STAGING: 이벤트 정규화, 시간 변환, 아이템 최신 스냅샷, 카테고리 트리 평탄화
- DATA MART: 이벤트/세션 grain 고정, dim/fact 분리, 세션 규칙 SQL 명시
- QA 5종: domain / integrity / null / rowcount / KPI range(0~1)
