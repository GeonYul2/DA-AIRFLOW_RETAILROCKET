...
핵심은 배치 성공이 아니라, **KPI 이전 행동로그 정합성 게이트 + KPI 이후 대사 검증**을 고정해 같은 데이터에서 같은 KPI가 나오게 만드는 것입니다.
...
| PRE-MART QA | KPI 산출 직전 마트 정합성 검증 | event/session 대사 + stg/fact null 체크를 통과한 실행만 KPI 계산 진행 |
| KPI | 지표 계산 일관성 확보 | Funnel/Cohort/CRM 계산을 별도 테이블로 분리해 분자/분모 정의를 단일화 |
| POST-QA | KPI 산출 후 결과 검증 | KPI sanity + source 대사 + monotonic 검증을 통과한 실행만 export 허용 |
