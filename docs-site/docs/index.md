...
<details>
<summary><strong>EDA 요약 보기</strong></summary>

- 전체 이벤트: **2,756,101**
- 고유 방문자: **1,407,580**
- 고유 아이템: **235,061**
- 기간: **2015-05-03 ~ 2015-09-18 (UTC date)**
- transaction 이벤트 `transaction_id` 누락: **0건**
...
- **RAW**
  - 왜: 원본이 바뀌면 원인 추적이 어려움
  - 어떻게: `events.csv`, `item_properties*.csv`, `category_tree.csv`를 원본 그대로 적재
  - 코드: `scripts/load_raw/load_retailrocket.py`, `sql/retailrocket/00_ddl/001_create_tables.sql`

- **STAGING**
  - 왜: 타입/시간/속성 포맷 차이로 집계 편차 발생
  - 어떻게: 이벤트 타입 정규화, `timestamp_ms → event_ts/event_date`, 속성 스냅샷, 카테고리 평탄화
  - SQL: `sql/retailrocket/10_staging/001~003_*.sql`

- **DATA MART**
  - 왜: 세션 기준이 바뀌면 CVR 해석이 달라짐
  - 어떻게: 세션 경계 규칙(첫 이벤트/날짜 변경/30분 초과)을 SQL로 고정
  - SQL: `sql/retailrocket/20_mart/010_fact_rr_events.sql`, `020_fact_rr_sessions.sql`

- **KPI**
  - 왜: KPI 값보다 분자/분모 정의의 일관성이 중요함
  - 어떻게: Funnel/Cohort/CRM 계산을 테이블 단위로 분리
  - SQL: `sql/retailrocket/30_kpi/001~004_*.sql`

- **QA**
  - 왜: DAG success가 데이터 신뢰를 보장하지 않음
  - 어떻게: 5개 품질 체크를 모두 통과한 경우에만 EXPORT 진행

- **EXPORT**
  - 왜: 분석 결과를 실행 가능한 포맷으로 전달해야 후속 액션이 빨라짐
  - 어떻게: QA 통과 실행에서만 CSV 3종 + summary TXT 생성
  - 코드: `scripts/export_rr_*.py`, `scripts/write_rr_pipeline_summary.py`
...
<figure>
  <div class="dag-scroll">
    <a href="assets/rr_funnel_daily-graph.png" target="_blank" rel="noopener">
      <img class="dag-wide" src="assets/rr_funnel_daily-graph.png" alt="Airflow DAG run success graph">
    </a>
  </div>
  <figcaption>Airflow DAG 실행 그래프(좌우 스크롤 지원). 이미지를 클릭하면 원본 크기로 확대해 볼 수 있습니다.</figcaption>
</figure>
